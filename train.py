...
class TrainingConfig:
    ...
    data_dir = "D:/document_denoiser/data"
    output_dir = "D:/document_denoiser/data"
...
class DocDenoiserDataset(torch.utils.data.Dataset):
    ...
    def get_test_image(self, index):
        ...
        noisy_image = torchvision.io.read_image(
            os.path.join(f'{config.data_dir}/test', filename)
        )
    ...
    def __getitem__(self, index):
        ...
        noisy_image = torchvision.io.read_image(
            os.path.join(f'{config.data_dir}/train', filename)
        )
        ...
        clean_image = torchvision.io.read_image(
            os.path.join(f'{config.data_dir}/train_cleaned', filename)
        )
...
class DocDenoiserDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: str):
        if stage == "fit" or stage == "validate":
            all_filenames = [
                filename
                for filename in os.listdir(f'{config.data_dir}/train')
                if os.path.isfile(os.path.join(f'{config.data_dir}/train', filename)) and
                os.path.isfile(os.path.join(f'{config.data_dir}/train_cleaned', filename))
            ]
        ...
        if stage == 'test':
            test_filenames = [
                filename
                for filename in os.listdir(f'{config.data_dir}/test')
                if os.path.isfile(os.path.join(f'{config.data_dir}/test', filename))
            ]
